...
namespace StaticViewLocatorGenerator;
...
public sealed class StaticViewLocatorGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterPostInitializationOutput(static ctx =>
            ctx.AddSource("StaticViewLocatorAttribute.g.cs", SourceText.From(ATTRIBUTE_TEXT, Encoding.UTF8)));
            ...
            .CreateSyntaxProvider(
                ...
                static (generatorContext, cancellationToken) =>
                {
                    if (generatorContext.Node is not TypeDeclarationSyntax typeDeclaration ||
                        generatorContext.SemanticModel.GetDeclaredSymbol(typeDeclaration, cancellationToken) is not
        ...
        context.RegisterSourceOutput(inputs, static (sourceProductionContext, tuple) =>
        {
            ...
            if (classSource is not null)
            {
                sourceProductionContext.AddSource(
                    $"{locatorSymbol.Name}_StaticViewLocator.g.cs",
        ...
        GeneratorOptions options)
    {
        ...
        source.AppendLine();
        source.AppendLine("    private static Dictionary<Type, Type> s_views = new()");
        source.AppendLine("    {");
        ...
        foreach (var viewModelSymbol in relevantViewModels)
        {
            ...
            source.AppendLine($"        [typeof({classNameViewModel})] = typeof({classNameView}),");
        }
            ...
            source.Append(
                """
                    ...
                    public Control? Build(object? data)
                    {
                        ...
                        if (TryGetViewType(type, out var viewType))
                        {
                            return _serviceProvider.GetRequiredService(viewType) as Control;
                        }

                        if (TryGetMissingView(type, out var missingStr))
                        {
                            return new TextBlock { Text = missingStr };
                        }

                        return new TextBlock { Text = $"Not Found: {type.FullName}'s View" };
                    }
        ...
        source.Append(
            $$"""
                ...
                private static bool TryGetViewType(Type? type,[NotNullWhen(true)] out Type? viewType)
                {
                    if (type is null)
                    {
                        viewType = null;
                    ...
                    if (TryGetViewTypeForType(type, out viewType))
                    {
                        return true;
                    }
                    ...
                    {
                        if (TryGetViewTypeForType(current, out viewType))
                        {
                            return true;
                        }
                    }
                ...
                private static bool TryGetViewTypeForType(Type type,[NotNullWhen(true)] out Type? viewType)
                {
                    if (s_views.TryGetValue(type, out viewType))
                    {
                        return true;
                    }

                    if (type.IsGenericType && s_views.TryGetValue(type.GetGenericTypeDefinition(), out viewType))
                    {
                        return true;
                    }
                ...
                private static bool TryGetMissingView(Type? type,[NotNullWhen(true)] out string? missingStr)
                {
                    if (type is null)
                    {
                        missingStr = null;
                    ...
                    if (TryGetMissingViewForType(type, out missingStr))
                    {
                        return true;
                    }
                    ...
                    for (var current = type.BaseType; current is not null; current = current.BaseType)
                    {
                        if (TryGetMissingViewForType(current, out missingStr))
                        {
                            return true;
                        }
                    }
                ...
                private static bool TryGetMissingViewForType(Type type,[NotNullWhen(true)] out string? missingStr)
                {
                    if (s_missingViews.TryGetValue(type, out missingStr))
                    {
                        return true;
                    }

                    if (type.IsGenericType && s_missingViews.TryGetValue(type.GetGenericTypeDefinition(), out missingStr))
                    {
                        return true;
                    }
                ...
                public {{classNameLocator}}(IServiceProvider serviceProvider)
                {
